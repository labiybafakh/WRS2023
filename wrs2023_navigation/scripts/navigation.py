...
class MONITORING(State):
  ...
  def callback(self, data):
    ...
    rospy.loginfo("Person %d", self.counter)
...
if __name__ == '__main__':
  ...
  with lifter_down:
    StateMachine.add('UP LIFTER', MOVE_LIFTER(0,0.5),\
      transitions={'succeeded':'succeeded','aborted':'aborted'})
  ...
  # go_to_wait = StateMachine(outcomes=['succeeded', 'preempted'])
  # with go_to_wait:
  #   StateMachine.add('DOWN LIFTER', MOVE_LIFTER(0,0.9),\
  #     transitions={'succeeded':'WAIT','aborted':'aborted'})
  #   StateMachine.add('WAIT', WAITING(1),\
  #     transitions={'succeeded':'succeeded', 'aborted':'preempted'})
  ...
  delay_2s = StateMachine(outcomes=['succeeded', 'aborted'])
  with delay_2s:
